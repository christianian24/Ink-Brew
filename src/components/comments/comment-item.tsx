...
export default function CommentItem({ 
  ...
  return (
    <div className="flex gap-4 group animate-in slide-in-from-bottom-2 fade-in duration-300">
      <div className="w-10 h-10 rounded-full bg-coffee-200 flex items-center justify-center font-serif text-coffee-800 flex-shrink-0">
        {comment.user.name?.[0]?.toUpperCase() || "U"}
      </div>
      
      <div className="flex-1 space-y-2">
        <div className="bg-coffee-50 rounded-2xl rounded-tl-none p-4 border border-coffee-100">
          <div className="flex items-center justify-between mb-2">
            <span className="font-bold text-coffee-900 text-sm">{comment.user.name}</span>
            <span className="text-xs text-coffee-400">
              {new Date(comment.createdAt).toLocaleDateString()}
            </span>
          </div>
          <p className="text-coffee-800 text-sm leading-relaxed whitespace-pre-wrap">
            {comment.content}
          </p>
        </div>

        <div className="flex items-center gap-4 px-2 text-xs font-medium text-coffee-500">
          <form action={() => {
          ...
          <button 
            ...
            className="flex items-center gap-1.5 hover:text-coffee-950 transition-colors"
          >
        ...
        {isReplying && (
          <form onSubmit={handleReplySubmit} className="mt-4 flex gap-3 animate-in slide-in-from-top-2 fade-in">
            <CornerDownRight className="w-5 h-5 text-coffee-300 mt-2" />
            <div className="flex-1 space-y-2">
              <textarea 
                value={replyText}
                onChange={(e) => setReplyText(e.target.value)}
                placeholder="Write a reply..."
                className="w-full bg-white border border-coffee-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-coffee-300 min-h-[80px]"
                autoFocus
              />
              <div className="flex justify-end gap-2">
                ...
                <Button type="submit" size="sm" disabled={isPending || !replyText.trim()} className="bg-coffee-800 hover:bg-coffee-900">
        ...
        {comment.replies && comment.replies.length > 0 && (
          <div className="mt-4 pt-4 border-l-2 border-coffee-100 pl-4 space-y-6">
